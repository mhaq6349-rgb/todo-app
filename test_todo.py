...
def no_error_toast(page):
    ...
    errs = [t for t in toasts if any(w in t.lower() for w in ['failed', 'cannot', 'error', 'offline'])]
    return len(errs) == 0
...
with sync_playwright() as p:
    ...
    check('no error toasts', no_error_toast(page))

    print('\n=== Add Task (no errors) ===')
    page.fill('#input', 'Buy groceries')
    page.click('#addBtn')
    page.wait_for_timeout(500)
    check('added', count(page) == 1, str(count(page)))
    check('no error toasts after add', no_error_toast(page))
    check('text matches', page.locator('.item').first.locator('.text').text_content() == 'Buy groceries')
    ...
    page.wait_for_timeout(500)
    check('second added', count(page) == 2, str(count(page)))
    check('no error toasts after second', no_error_toast(page))
    due_spans = page.locator('.date.due')
    check('due badge visible', due_spans.count() >= 1, str(due_spans.count()))

    page.fill('#input', 'Read a book')
    page.click('#addBtn')
    page.wait_for_timeout(500)
    check('third added', count(page) == 3, str(count(page)))
    check('no error toasts after third', no_error_toast(page))
    ...
    check('no error toasts after clear', no_error_toast(page))
    ...
    check('no error toasts after undo', no_error_toast(page))
    ...
    check('no error toasts after reload', no_error_toast(page))
